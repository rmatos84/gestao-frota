...
export default function App() {
  ...
  const saveMotorista = async () => {
    ...
    try {
      ...
      setFormMotorista(emptyMotorista);
  ...
  const runAI = async () => {
    ...
    setAiAnalysis("");
    setTab("ia");
    const resumo = stats.map(s =>
      `${s.nome}: ${s.kmTotal}km, ${s.kml} km/L, ${s.viagens} viagem(ns), ${s.litros.toFixed(0)}L${s.gasto > 0 ? `, R$${s.gasto.toFixed(2)}` : ""}`
    ).join("\n");
    try {
  ...
  const tabs = [["dashboard","📊 Dashboard"],["registros","⛽ Abastecimentos"],["motoristas","👤 Motoristas"],["veiculos","🚗 Veículos"],["ia","🤖 IA"]];
  ...
  return (
    <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif", background: "#0a0f1a", minHeight: "100vh", color: "#e2e8f0" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "20px 16px" }}>
        <div style={{ display: "flex", gap: 3, background: "#0f172a", borderRadius: 12, padding: 4, marginBottom: 24, border: "1px solid #1e293b", overflowX: "auto" }}>
          {tabs.map(([key, label]) => (
        ...
        {!loading && tab === "dashboard" && (
          <div>
            ...
            {stats.length === 0 ? <div style={{ background:"#0f172a", border:"1px solid #1e293b", borderRadius:16, padding:40, textAlign:"center", color:"#475569" }}>Nenhum registro ainda. Cadastre motoristas, veículos e abastecimentos.</div> : (
